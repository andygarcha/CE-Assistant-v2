...
def get_banned_games() -> list[str] :
    ...
    BANNED_GAMES = SpreadsheetHandler.get_sheet_data(
        SpreadsheetHandler.CE_SHEET_BANNED_GAMES_RANGE, 
...
def achievements_are_equal(old_achievements : list[str] | None, new_achievements : list[str] | None) -> bool :
    ...
    if old_achievements is None and new_achievements is not None:
        return False
    if old_achievements is not None and new_achievements is None:
        return False
    if old_achievements is None and new_achievements is None:
        return True
