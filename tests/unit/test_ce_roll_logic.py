...
class TestIsWonSolo:
    ...
    def test_expired_roll_does_not_win(self):
        ...
        user = _completed_user(GAME_ID_A)
        roll = make_roll(
            roll_name="One Hell of a Day", games=[GAME_ID_A], due_time=_PAST
        )
        assert roll.is_won([game], user) is False
    ...
    def test_multi_game_roll_all_completed_wins(self):
        ...
        uobj_a = make_user_objective(
            ce_id=OBJ_ID, game_ce_id=GAME_ID_A, user_points=100
        )
        uobj_b = make_user_objective(
            ce_id=OBJ_ID, game_ce_id=GAME_ID_B, user_points=100
        )
...
class TestCasinoIncrease:
    ...
    def test_destiny_alignment_relative_tiers(self, points, expected):
        ...
        roll = make_roll(
            roll_name="Destiny Alignment", games=[GAME_ID_A], tier_num=game.tier_num
        )
