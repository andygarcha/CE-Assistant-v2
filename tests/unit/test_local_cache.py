...
class TestInit:
    ...
    def test_wal_mode(self):
        ...
        try:
            mode = (
                LocalCache.get_connection().execute("PRAGMA journal_mode").fetchone()[0]
            )
            assert mode == "wal"
        finally:
            _teardown(tmpdir)
...
class TestGames:
    ...
    def test_bulk_upsert(self):
        ...
        try:
            rows = [
                {**GAME_ROW, "ce_id": f"g{i}", "name": f"Game {i}"} for i in range(5)
            ]
    ...
    def test_get_by_user(self):
        ...
        try:
            ...
            LocalCache.upsert_roll(
                {**ROLL_ROW, "id": "r2", "user1_ce_id": "u3", "user2_ce_id": None}
            )
    ...
    def test_get_checkable(self):
        ...
        try:
            for status in ["current", "pending", "won", "removed"]:
                LocalCache.upsert_roll(
                    {**ROLL_ROW, "id": f"r-{status}", "status": status}
                )
    ...
    def test_get_by_event_names(self):
        ...
        try:
            for name in ["Soul Mates", "Triple Threat", "One Hell of a Day"]:
                LocalCache.upsert_roll(
                    {**ROLL_ROW, "id": f"r-{name}", "event_name": name}
                )
            result = LocalCache.get_rolls_by_event_names(
                ["Soul Mates", "Triple Threat"]
            )
            assert len(result) == 2
...
class TestTier:
    ...
    def test_upsert_overwrites_price(self):
        ...
        try:
            ...
            LocalCache.upsert_tier_bulk(
                [{"ce_id": "g1", "price": 4.99, "sh_hours": 5.0}]
            )
            result = LocalCache.get_tier_all()
            assert len(result) == 1
...
class TestBulkUpsertEdgeCases:
    ...
    def test_large_batch_games(self):
        ...
        try:
            rows = [
                {**GAME_ROW, "ce_id": f"g{i}", "name": f"Game {i}"} for i in range(200)
            ]
            LocalCache.upsert_games_bulk(rows)
...
class TestDataIntegrity:
    ...
    def test_rolls_by_user_finds_as_user2(self):
        ...
        try:
            LocalCache.upsert_roll(
                {**ROLL_ROW, "id": "r1", "user1_ce_id": "u1", "user2_ce_id": "u2"}
            )
    ...
    def test_rolls_by_user_no_duplicates_when_both_fields_match(self):
        ...
        try:
            LocalCache.upsert_roll(
                {**ROLL_ROW, "id": "r1", "user1_ce_id": "u1", "user2_ce_id": "u1"}
            )
