...
def _uncleared_po(obj_id: str = "obj-uncleared-000000000000") -> object:
    ...
    return make_objective(
        ce_id=obj_id, point_value=0, obj_type="Primary",
        name="PO (UNCLEARED)", game_ce_id=GAME_ID
    )


def _upo(points: int, obj_id: str = OBJ_ID) -> object:
    """User primary objective with given points."""
    return make_user_objective(ce_id=obj_id, game_ce_id=GAME_ID, obj_type="Primary", user_points=points)
...
class TestGetUserPoints:
    def test_sums_all_user_objectives(self):
        ug = make_user_game(user_objectives=[
            make_user_objective(user_points=10),
            make_user_objective(ce_id="obj-0002-0000-0000-000000000000", user_points=20),
        ])
    ...
    def test_single_objective(self):
        assert make_user_game(user_objectives=[make_user_objective(user_points=15)]).get_user_points() == 15
...
class TestHasCompletedObjective:
    ...
    def test_empty_objectives_returns_false(self):
        assert make_user_game(user_objectives=[]).has_completed_objective(OBJ_ID, 10) is False
...
class TestIsCompleted:
    ...
    def test_user_po_count_mismatch_not_completed(self):
        ...
        game = make_game(ce_id=GAME_ID, objectives=[
            _po(10, "obj-a"),
            _po(20, "obj-b"),
        ])
    ...
    def test_game_not_in_database_list_returns_false(self):
        other_game = make_game(ce_id="other-00-0000-0000-000000000000", objectives=[_po(10)])
        ug = make_user_game(ce_id=GAME_ID, user_objectives=[_upo(10)])
    ...
    def test_completed_with_uncleared_po_included(self):
        ...
        ug = make_user_game(ce_id=GAME_ID, user_objectives=[
            _upo(10),
            make_user_objective(ce_id=obj_id2, game_ce_id=GAME_ID, obj_type="Primary", user_points=0),
        ])
        assert ug.is_completed(game) is True
    ...
    def test_secondary_objectives_do_not_count(self):
        ...
        game = make_game(ce_id=GAME_ID, objectives=[
            _po(10),
            make_objective(ce_id="obj-so", point_value=50, obj_type="Secondary", game_ce_id=GAME_ID),
        ])
        ug = make_user_game(ce_id=GAME_ID, user_objectives=[_upo(10)])
        assert ug.is_completed(game) is True
