...
async def solo_roll(interaction : discord.Interaction, event_name : hm.SOLO_ROLL_EVENT_NAMES, 
    ...
    match(event_name) :
        ...
        case "One Hell of a Month" :
            ...
            # -- grab games --
            logger.debug(f'rolling at time {datetime.datetime.now()}')
            rolled_games : list[str] = []
            valid_categories = list(get_args(hm.CATEGORIES))
            ...
            for i in range(5) :
                ...
                for j in range(5) : #roll 5 games from the selected category
                    logger.debug(f'rolling game {(i + 1) * (j + 1)}')
                ...
                #debugging
                logger.info(f"User ({user.display_name}) rolled the {selected_category} category, with rolled games {rolled_temp}")
                ...
                if None in rolled_temp and failed_category == None: #if not enough rolls in a given category, and this is the first failed category, remove and try again
                    ...
                    #debugging
                    logger.warning(f"There were not enough valid rolls for user ({user}) in the {failed_category} category. "
                    ...
                    #debugging                    
                    logger.info(f"User ({user}) re-rolled into the {selected_category} category, with rolled games {rolled_temp}")
                ...
                #debugging
                logger.info(f"The following games from the {selected_category} category were added to the user's ({user.ce_id}) Hell Month roll list: {rolled_temp}")
